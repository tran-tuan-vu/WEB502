...
function findMinMax(list: IProduct[]): { min: IProduct | null, max: IProduct | null } {
    if (list.length === 0) {
...
interface IMotorezedVehicle extends IVehicle {
    fuelType: FuelType;
}

function calculateTravelTime(vehicle: IVehicle, distance: number): number {
    if (vehicle.speed <= 0) {
...
const motorizedVehicles: IMotorezedVehicle[] = [
